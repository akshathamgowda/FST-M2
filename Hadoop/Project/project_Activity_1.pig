-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/akshatham/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/akshatham/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/akshatham/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/akshatham/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/akshatham/outputs' USING PigStorage('\t');
